...
export const askDify = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<DifyResult> => {
    ...
    // 🚀 DIRECT CLOUD BRIDGE: Spaces cleaned out and verified!
    const GROQ_API_KEY = "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"; 
    
    // Fallback key check - syntax fixed cleanly
    if (!GROQ_API_KEY || GROQ_API_KEY === "gsk_...") {
      throw new Error("Please paste a valid Groq API key into dify.functions.ts");
    }
